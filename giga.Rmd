---
title: "Adjusted Grotta Bars for the display of causal effects with ordinal outcomes"
authors: 'Rodrigo Huerta Gutiérrez, Marco Piccininni, Tobias Kurth'
...
```{r}
grottaBar(tableOrdinal, groupName = "group", scoreName = "mRS")
```
...
```
## Constructing adjusted Grotta Bar accounting for confounding through Inverse Probability of Treatment Weightening 

```{r, echo = FALSE}
# Releveling variables
tab2 <- IST3%>%
  mutate(Home= dplyr::recode(Home, `1`= "Home", `0`="Not home"),
         mrs = factor(ohs6, levels = c(0,1,2,3,4,5)))

# Generating table
rankin2 <- table(tab2$ohs6, tab2$Home)

# Giving names to colums in table
names(dimnames(rankin2)) <- c("mRS", "Home")

# Creating Grotta bars
unadjustedGrotta <- rankinPlot::grottaBar(rankin2, groupName = "Home", 
                       scoreName = "mRS",
...
adjustedGrotta <- rankinPlot::grottaBar(rankin3, groupName = "Home",
                      scoreName = "mRS", 
                      printNumbers = "percentage")

### Unadjusted Grotta Bars 
unadjustedGrotta
